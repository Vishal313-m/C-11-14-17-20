...
/*And this is another kind of smart pointer we have in C++ The special thing about this pointer is that we can have multiple pointers pointing to the same data
that we have in memory.

For example, we have a pointer one which is a shared pointer.It is pointing to our data.
We have a pointer two, which is also pointed to the same data.
We have pointer three, which is pointing to the same data.
We have pointer four, which is pointing to the same data.

One special thing about shared pointers is that there is some kind of reference count which is going
to keep track of how many pointers actually point to the same data.
For example, if we set up pointer one here, the reference count is going to be one.
If we set up a pointer two, the reference count is going to go up and be two.
If we set up pointer three, the reference count is going to be three.
If we set up pointer four here, the reference count is going to go to four.

If any of these pointers goes out of scope, the reference count is going to decrement.
For example, if Pointer three here goes out of scope, the reference count is going to decrement and be three.

If for example, pointer one here goes out of scope, then the reference count is going to decrement and be two.

If pointer two goes out of scope, for example, the reference count is going to go down to one because 
we still have only one pointer pointing to the data and then when the pointer four goes out of scope,
the reference count is going to go down to zero and that's when the data in memory is going to be wiped out and released.

So shared pointers are only going to release the memory when the reference count goes to zero at any
point when we have a single pointer or more pointers pointing to the data, the data is not going to
be released if one of those pointers goes out of scope.This is the main design principle of shared pointers here.*/
...
int_ptr_4 : 0x560baf29deb0
int_ptr_4.get() : 0x560baf29deb0
int_ptr_4->bool : true
...
Casting to bool and using in if statements...
dog_ptr_4 : 0x55555b36eeb0
dog_ptr_4.get() : 0x55555b36eeb0
dog_ptr_4->bool : 1
dog_ptr_5->bool : 0
